...
pub async fn echo(listener: TcpListener) -> Result<(), Error> {
    loop {
        // Accept a new connection on the async executer
        let (socket, _) = listener.accept().await?;

        // Offload the blocking I/O to the blocking pool
        tokio::task::spawn_blocking(move || -> Result<(), Error> {
            // Convert to a std::net::TcpStream (blocking)
            let mut socket = socket.into_std()?;
            // Make sure it really blocks (not strictly needed)
            socket.set_nonblocking(false)?;

            // Echo everything we need
            let mut buffer = Vec::new();
            socket.read_to_end(&mut buffer)?;
            socket.write_all(&buffer)?;
            Ok(())
        })
        .await??; // propagate any errors from teh spawned task
    }
}
