...
pub async fn echo(listener: TcpListener) -> Result<(), anyhow::Error> {
    // 'listener' is moved into the loop; we can keep it mutable if we ever want
    // to drop it early, bu there we just loop forever.
    loop {
        // Wait for the next incoming connection.
        let (mut socket, _addr) = listener.accept().await?;

        // Spawn a lightweight task for this connection; it runs concurrently
        ...
        tokio::spawn(async move {
            ...
            // 'copy' will read until EOF (the client shuts down its write side)
            // and write the same data back to the client.
            // We ignore the result - if it fails we just drop the task.
